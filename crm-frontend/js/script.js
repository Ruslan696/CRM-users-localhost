document.addEventListener('DOMContentLoaded', async () => {
  ...
  // async function postToServer(something) {
  ...
  // async function patchToServer(something, id) {
  ...
  function modalOpen() {
    ...
    document.querySelector('.new-client-box').classList.add('modal-open');
  ...
  function modalClose() {
    ...
    document.querySelector('.new-client-box').classList.remove('modal-open');
  ...
  async function changeClientModal(id) {
    ...
    if (!response.ok) {
      ...
      document.querySelector('.add-contact__btn').classList.add('first-click')
      if (document.querySelector('.new-client-form.new-client-submit')) {
        document.querySelector('.new-client-form.new-client-submit').classList.remove('new-client-submit')
      }
      ...
      let modalBox = document.querySelector('.new-client-box')
      let clientTitle = document.querySelector('.new-client-title')
  ...
  async function newClientSubmit() {
    ...
    let surnameClient = document.getElementById('new-client-surname').value.trim().split(' ').join('').toLowerCase();
    let nameClient = document.getElementById('new-client-name').value.trim().split(' ').join('').toLowerCase();
    let lastNameClient = document.getElementById('new-client-last-name').value.trim().split(' ').join('').toLowerCase();
    ...
    for (const contact of allNewContacts) {
      ...
      if (attr === 'tel') {
        ...
        client.contacts.push(contactJbject)
      }
      if (attr === 'email') {
    ...
    await postToServer();
    document.querySelector('.new-client-form').classList.remove('new-client-submit');
  ...
  // Форма добавления нового клиента(Модальное окно)
  function addClientForm() {
    const newClientBox = document.createElement('div');
    newClientBox.classList.add('new-client-box')
    ...
    newClientTitle.classList.add('new-client-title')
    newClientTitle.textContent = 'Новый клиент'
    const newClientForm = document.createElement('form');
    newClientForm.id = 'client-form'
    newClientForm.classList.add('new-client-form');
    const surnameInput = document.createElement('input');
    surnameInput.classList.add('new-client-form__input');
    surnameInput.id = 'new-client-surname'
    const labelForSurname = document.createElement('label')
    labelForSurname.classList.add('new-client-form__label', 'label-for-surname')
    labelForSurname.setAttribute('for', 'new-client-surname')
    labelForSurname.innerHTML = `Фамилия<span style = "color: #9873FF;">*</span>`;

    const nameInput = document.createElement('input');
    nameInput.classList.add('new-client-form__input');
    nameInput.id = 'new-client-name'
    const labelForName = document.createElement('label')
    labelForName.classList.add('new-client-form__label', 'label-for-name')
    labelForName.setAttribute('for', 'new-client-name')
    labelForName.innerHTML = `Имя<span style = "color: #9873FF;">*</span>`;

    const lastNameInput = document.createElement('input');
    lastNameInput.classList.add('new-client-form__input');
    lastNameInput.id = 'new-client-last-name';
    const labelForlastName = document.createElement('label')
    labelForlastName.classList.add('new-client-form__label', 'label-for-last-name')
    labelForlastName.setAttribute('for', 'new-client-last-name')
    labelForlastName.innerHTML = 'Отчество';
    ...
    addContactBtn.innerHTML = `<svg class="add-contact__svg" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    cancelBtn.classList.add('cancel-btn', 'link-reset');
    cancelBtn.textContent = 'Отмена';
    const exitBtn = document.createElement('a')
    exitBtn.classList.add('new-client-exit-btn', 'link-reset')
    ...
    deleteBtn.classList.add('new-client-form__delete-btn')
    ...
    addContactBtn.addEventListener('click', (e) => {
      ...
      newContactInput.classList.add('new-contact-input')
    ...
    newClientForm.append(labelForSurname);
    newClientForm.append(surnameInput);
    newClientForm.append(labelForName);
    newClientForm.append(nameInput);
    newClientForm.append(labelForlastName);
    newClientForm.append(lastNameInput);
    newClientForm.append(addContact);
    newClient.append(errorStatus)
    newClientForm.append(saveBtn);
    newClient.append(newClientTitle);
    newClient.append(newClientForm);
    newClient.append(cancelBtn);
    newClient.append(deleteBtn)
    newClient.append(exitBtn)
    newClientBox.append(newClient)
    document.body.append(newClientBox);
  };
  addClientForm()
  ...
  function createTable(array) {
    ...
    for (const client of array) {
      ...
      tRow.classList.add('table-tr')

      let cellId = document.createElement('td')
      cellId.classList.add('table-td', 'table-td_id')
      cellId.style.color = '#B0B0B0'
      cellId.textContent = `${client.id}`
      tRow.append(cellId)

      let cellName = document.createElement('td');
      cellName.classList.add('table-td', 'table-td_name')
      ...
      cellCreatedAt.classList.add('table-td', 'table-td_create')
      ...
      cellCreatedAtDate.innerHTML = `<nobr>${dateClientCreated}-${monthClientCreated}-${clientCreatedAt.getFullYear()}</nobr>`
      ...
      tRow.append(cellCreatedAt)


      const cellUpdateAt = document.createElement('td')
      cellUpdateAt.classList.add('table-td', 'table-td_change')
      ...
      cellUpdateAtDate.innerHTML = `<nobr>${dateClientUpdate}-${monthClientUpdate}-${clientUpdateAt.getFullYear()}</nobr>`
      ...
      tRow.append(cellUpdateAt)


      let cellContacts = document.createElement('td')
      cellContacts.classList.add('table-td', 'table-td_contacts')
      let clientContacts = []
      if (client.contacts) {
      ...
      if (clientContacts.length == 0) {
        tRow.append(cellContacts)
      } else {
        for (const clientContact of clientContacts) {
          let contactLink = document.createElement('a')
          contactLink.classList.add('table-contacts-link', 'link-reset');
      ...
      if (clientContacts.length <= 5) {
        ...
        let thisContactsLinks = cellContacts.querySelectorAll('.table-contacts-link');
        ...
        contactLinkMore.classList.add('table-contacts-link_more')
        contactLinkMore.textContent = `+${clientContacts.length - 4}`;
        cellContacts.append(contactLinkMore);
        tRow.append(cellContacts)
        cellContacts.querySelector('.table-contacts-link_more').addEventListener('click', (e) => {
          e.preventDefault()
      ...
      cellActions.classList.add('table-td', 'table-td_actions')
      let changeClientBtn = document.createElement('button')
      changeClientBtn.classList.add('change-client-btn', 'btn-reset')
      ...
      deleteClientBtn.classList.add('delete-client-btn', 'btn-reset')
      ...
      tRow.append(cellActions)

      table.append(tRow)
    };
    // Кнопка изменения клиента
    document.querySelectorAll('.change-client-btn').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        let target = e.target.closest('.change-client-btn')
        ...
        let idClient = target.parentNode.parentNode.querySelector('.table-td_id').textContent
    ...
    document.querySelectorAll('.delete-client-btn').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        const target = e.target.closest('.delete-client-btn');
        let deleteBtn = document.querySelector('.delete-box__delete-btn')
        deleteBtn.removeAttribute('id')
  ...
  document.querySelector('.new-client-exit-btn').addEventListener('click', (e) => {
    e.preventDefault();
    modalClose();
  });
  ...
  function deleteClient() {
    ...
    document.querySelector('.new-client-box').append(deleteBox)
  }
  deleteClient();
  ...
  async function loadTable(sortVar, field) {
    if (!document.querySelector('.table-tr')) {
      document.querySelector('.add-client-btn').classList.add('is-hidden')
      preloadTable();
    } else {
    ...
    document.querySelector('.add-client-btn').classList.remove('is-hidden')
  }
  ...
  document.querySelector('.add-client-btn').addEventListener('click', (e) => {
    e.preventDefault();
    if (document.querySelector('.form-input-error')) {
      document.querySelectorAll('.form-input-error').forEach((el) => {
        el.remove()
    ...
    document.querySelector('.new-client-form__delete-btn').classList.add('is-hidden')
    ...
    document.querySelector('.new-client-box').classList.add('modal-open')
    document.querySelector('.new-client').classList.add('modal-open')
    document.querySelector('.new-client-form').classList.add('new-client-submit')
    document.querySelectorAll('.new-client-form__input').forEach((el) => {
      el.value = '';
    ...
    if (document.querySelector('.new-client-form.change-client-submit')) {
      document.querySelector('.new-client-form.change-client-submit').classList.remove('change-client-submit')
    }
  ...
  sotrByField('.clients-table__thead-th_id', 'id');
  sotrByField('.clients-table__thead-th_create', 'createdAt');
  sotrByField('.clients-table__thead-th_change', 'updatedAt');

  // Сортировка по имени
  document.querySelector('.clients-table__thead-th_name').addEventListener('click', () => {
    if (!document.querySelector('.table-tr')) {
      document.querySelector('.add-client-btn').classList.add('is-hidden')
      preloadTable();
    } else {
    ...
    document.querySelector('.clients-table__thead-th_name').classList.toggle('sort-down')
    if (document.querySelector('.clients-table__thead-th_name.sort-down')) {
  ...
  // Поисковая строка в шапке c отрисовкой таблицы по совпадениям
  // function inputSearch() {
  //   let timeoutID
  //   document.querySelector('.header__input').addEventListener('input', () => {
  //     clearInterval(timeoutID)
  //     async function inputText() {
  //       if (!document.querySelector('.table-tr') && !document.querySelector('.table-preload')) {
  //         document.querySelector('.add-client-btn').classList.add('is-hidden')
  //         preloadTable();
  //       } else {
  //         document.querySelectorAll('.table-tr').forEach((el) => {
  ...
  //       const clientsFromServer = await response.json();
  //       clients = []
  //       let text = document.querySelector('.header__input').value.toLowerCase()
  //       for (const client of clientsFromServer) {
  //         const getFullName = `${client.surname} ${client.name} ${client.lastName}`
  //         const fullName = getFullName.toLowerCase()
  ...
  //           document.querySelector('.add-client-btn').before(notFound)
  //         }
  ...
  function autocomplete() {
    ...
    input.addEventListener('input', (e) => {
      let clients = [];
      const allClients = document.querySelectorAll('.table-td_name');
      ...
      for (let i = 0; i < clients.length; i++) {
        if (clients[i].toLowerCase().includes(inputValue.toLowerCase())) {
          ...
          autoItem.addEventListener('click', (e) => {
            ...
            closeAllItems()
            document.querySelectorAll('.table-td_name').forEach((el) => {
  ...
  document.querySelector('.new-client-box').addEventListener('click', (e) => {
    e.preventDefault()
    const target = e.target;
    if (target.closest('.new-client-form__label')) {
      const attr = target.closest('.new-client-form__label').getAttribute('for')
    ...
    if (!target.closest('.new-client') && !target.closest('.delete-box') && !target.closest('.add-contact')) {
